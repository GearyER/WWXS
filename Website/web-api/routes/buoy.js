...
/* Get a specific buoy owner from MAC address */
router.get('/:mac/owner', async (req, res) => {
  const query = {};
  query.where = { mac: req.params.mac };
  query.include = [
    {
      model: req.context.models.Group
    }
  ...
  // Find the buoy using the MAC address
  req.context.models.Buoy.findOne({query})
  .then((buoy) => {
    if (buoy) {
      // Get the user associated with this buoy
      req.context.models.User.findByPk(buoy.userId)
        .then((user) => {
          if (user) {
...
router.post('/', async (req, res) => {
  req.context.models.Buoy.bulkCreate(req.body)
    /* Successful insert */
